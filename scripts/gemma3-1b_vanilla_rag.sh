#!/bin/bash
# gemma3-1b — vanilla_rag baseline 评测
# 用法：
#   bash scripts/gemma3-1b_vanilla_rag.sh                          # 默认 GPU 0，全量
#   CUDA_VISIBLE_DEVICES=2 bash scripts/gemma3-1b_vanilla_rag.sh   # 指定 GPU
#   N_SAMPLES=10 bash scripts/gemma3-1b_vanilla_rag.sh             # smoke test
set -euo pipefail
...
COMMON="python -m evaluation.eval_baseline \
    --model-path hugglingface_model/gemma3-1b \
    --method vanilla_rag \
    --output-dir results/baseline \
    --n-samples $N_SAMPLES"

echo "=== gemma3-1b / vanilla_rag ==="
