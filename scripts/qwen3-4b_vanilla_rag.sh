#!/bin/bash
# qwen3-4b — vanilla_rag CoT 评测 (LLMLingua-2 压缩 + 中性 prompt + nothink)
# 用法：
#   bash scripts/qwen3-4b_vanilla_rag.sh                          # 默认 GPU 0，全量
#   CUDA_VISIBLE_DEVICES=2 bash scripts/qwen3-4b_vanilla_rag.sh   # 指定 GPU
#   N_SAMPLES=10 bash scripts/qwen3-4b_vanilla_rag.sh             # smoke test
set -euo pipefail
...
echo "=== qwen3-4b / vanilla_rag (CoT + LLMLingua-2) ==="
