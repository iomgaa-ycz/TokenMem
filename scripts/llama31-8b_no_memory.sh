#!/bin/bash
# llama31-8b — no_memory CoT 评测 (batch)
# 用法：
#   bash scripts/llama31-8b_no_memory.sh                          # 默认 GPU 0，全量
#   CUDA_VISIBLE_DEVICES=2 bash scripts/llama31-8b_no_memory.sh   # 指定 GPU
#   N_SAMPLES=10 bash scripts/llama31-8b_no_memory.sh             # smoke test
set -euo pipefail
