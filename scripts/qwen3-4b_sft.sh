#!/usr/bin/env bash
# qwen3-4b_sft.sh — Qwen3-4B News + Counterfactual SFT (LinearFusion gates only)
#
# 训练参数来自 DecoupledRAG (Lamb lr=1e-3, LinearLR warmup=10)
# 可训练参数: 仅 gate_crossattention (LinearFusion W_A + W_B), ~2.95M trainable params
# 基座 LLM 全部冻结 (4B params) — default baseline model
#
# 复现:
#   bash scripts/qwen3-4b_sft.sh
#   CUDA_VISIBLE_DEVICES=2 bash scripts/qwen3-4b_sft.sh
#   CUDA_VISIBLE_DEVICES=2,3 NUM_GPUS=2 bash scripts/qwen3-4b_sft.sh
set -euo pipefail
