#!/bin/bash
# qwen3-14b — vanilla_rag CoT 评测 (LLMLingua-2 压缩 + nothink + batch)
# 用法：
#   bash scripts/qwen3-14b_vanilla_rag.sh                          # 默认 GPU 0，全量
#   CUDA_VISIBLE_DEVICES=2 bash scripts/qwen3-14b_vanilla_rag.sh   # 指定 GPU
#   N_SAMPLES=10 bash scripts/qwen3-14b_vanilla_rag.sh             # smoke test
set -euo pipefail
...
COMMON="python -m evaluation.eval_baseline \
    --model-path hugglingface_model/qwen3-14B \
    --method vanilla_rag \
...
echo "=== qwen3-14b / vanilla_rag (CoT + LLMLingua-2) ==="
