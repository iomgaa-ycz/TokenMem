...
### 2.2 远程服务器 4090-serve

#### 2.2.1 4090-serve

> [!CRITICAL]
> **长时间训练实验在远程服务器运行**。本机（4070 Ti Super）可进行小规模实际测试，但不要长时间占用 GPU。

```yaml
## Remote Server
- gpu: remote
- SSH: ssh yuchengzhang@100.83.164.94
- GPU: 8x RTX 4090 (49GB)，使用空闲卡（优先 GPU 2/3/6）
- Conda: eval "$(/opt/miniconda3/bin/conda shell.bash hook)" && conda activate ExplicitLLM
- Code dir: /home/yuchengzhang/Project/Memory-LoRA/
- code_sync: none  # Syncthing 自动双向同步，无需手动同步
- swanlab: true
```
...
> [!CRITICAL]
> **适合大规模长时间训练，但是属于HPC集群，所有无法调试**。

**实验工作流**：
1. 本地调试 → Syncthing 自动同步代码到远程
2. `/run-experiment` SSH 到远程启动 screen 会话
3. `/monitor-experiment` SSH 检查训练状态
4. 结果 JSON 由 Syncthing 同步回本地 `results/`
5. 对于8B及以上的模型由人类手动在A100上运行
