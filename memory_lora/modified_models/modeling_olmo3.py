#                Modified from transformers/models/olmo3/modeling_olmo3.py
#                添加 cross-attention + LinearFusion gate 支持
#                用于 TokenMem 知识注入
# coding=utf-8
...
class Olmo3Attention(nn.Module):
    ...
    def __init__(self, config: Olmo3Config, layer_idx: int):
        ...
        # OLMo3 特有：QK norm 作用在完整投影维度上（不是 per-head）
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # OLMo3: QK norm 在 view/reshape 之前，作用在完整投影维度上
        query_states = self.q_norm(self.q_proj(hidden_states))
        query_states = query_states.view(hidden_shape).transpose(1, 2)

        # Cross-attention 分支: Q 来自 hidden_states, K/V 来自 encoder_hidden_states
        if is_cross_attention and encoder_hidden_states is not None:
            ...
            key_states = self.k_norm(self.k_proj(encoder_hidden_states))
        ...
        # Cross-attention 使用 eager 路径，无 causal mask
...
class Olmo3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Olmo3Config, layer_idx: int):
        ...
        # Cross-attention + LinearFusion gate（仅在配置启用且层号 <= 阈值时添加）
        self.add_cross_attention = getattr(
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.add_cross_attention and encoder_hidden_states is not None:
            residual = hidden_states
            cross_input = self.post_attention_layernorm(encoder_hidden_states)
...
class Olmo3Model(Olmo3PreTrainedModel):
    ...
    def forward(
        ...
        knowledge_outputs: Optional[list] = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> BaseModelOutputWithPast:
        ...
        for idx, decoder_layer in enumerate(
            ...
            # 知识注入：按层传递 encoder_hidden_states
            ...
            hidden_states = decoder_layer(
                ...
                encoder_hidden_states=layer_knowledge,
                **kwargs,
            )
