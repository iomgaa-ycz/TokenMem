"""RAG SFT 训练脚本 —— LoRA 微调基座模型，knowledge 通过 prompt 注入。

对照 baseline: 与 TokenMem (cross-attention 注入 knowledge) 保持相同训练超参:
- 优化器: Lamb (lr=1e-3, wd=0.0)
- 调度器: LinearLR 10-step warmup
- 精度: bf16 (via Accelerator)
- 差异: 可训练参数为 LoRA adapter (默认 v_proj, r=16)

用法:
    accelerate launch training/rag_sft.py \\
        --model-name-or-path hugglingface_model/qwen3-8B \\
        --train-jsonl data/train.jsonl \\
        --val-jsonl data/val.jsonl \\
        --ckpt-dir checkpoints/rag-sft-qwen3-8b
"""
...
from training.data_rag import make_rag_collate_fn  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    与 TokenMem sft.py 保持相同参数结构，额外增加 LoRA 相关参数。

    返回:
        argparse.Namespace: 解析后的参数命名空间
    """
    p = argparse.ArgumentParser(description="RAG SFT 训练脚本 (LoRA)")
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="HuggingFace 模型路径（相对路径如 hugglingface_model/qwen3-8B）",
    )
    p.add_argument(
    ...
    # --- LoRA 参数 ---
    p.add_argument(
        "--lora-rank", type=int, default=16, help="LoRA rank (默认: 16)"
    )
    p.add_argument(
        "--lora-alpha", type=int, default=16, help="LoRA alpha (默认: 16)"
    )
    p.add_argument(
        "--lora-dropout", type=float, default=0.0, help="LoRA dropout (默认: 0.0)"
    )
    p.add_argument(
        "--lora-target-modules",
        type=str,
    ...
    p.add_argument(
        ...
        type=int,
        default=1024,
        help="knowledge+prompt+answer 最大 token 长度 (默认: 1024)",
    )
    p.add_argument(
        "--knowledge-max-len",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="早停耐心值，0=不启用早停 (默认: 0)",
    )

    # --- Curriculum 训练 (Phase 2) ---
    p.add_argument(
        "--load-lora",
        type=str,
        default=None,
        help="加载已有 LoRA 权重的目录路径（用于 Phase 2 curriculum 训练）",
    )
    p.add_argument(
        "--cf-train-jsonl",
        type=str,
...
def save_checkpoint(
    ...
    """保存 LoRA adapter 权重和元信息。

    仅在主进程执行写入操作。使用 unwrap_model 获取 PEFT 模型后调用 save_pretrained。
...
def evaluate(
    ...
    参数:
        model: 被 accelerator.prepare 包装后的 PEFT 模型
...
def _log_param_stats(model: Any, accelerator: Accelerator) -> None:
    """统计并打印可训练 / 总参数量。
...
def train(args: argparse.Namespace) -> None:
    ...
    2. 加载基座模型 (frozen) + PEFT LoRA adapter
    3. 构建 Dataset + DataLoader + RAG collate_fn
    ...
    if accelerator.is_main_process:
        logger.info("=" * 60)
        logger.info("RAG SFT 训练开始 (LoRA on %s)", args.lora_target_modules)
        logger.info("=" * 60)
        logger.info(
    ...
    # --- 加载基座模型 ---
    if accelerator.is_main_process:
        logger.info("加载基座模型: %s", args.model_name_or_path)
    ...
    if args.load_lora:
        # Phase 2: 从已有 LoRA 权重加载
        model = PeftModel.from_pretrained(base_model, args.load_lora)
        # 确保 LoRA 参数可训练
        for name, param in model.named_parameters():
            if "lora_" in name:
                param.requires_grad = True
        if accelerator.is_main_process:
            logger.info("已加载 LoRA 权重: %s", args.load_lora)
    else:
    ...
    if args.cf_train_jsonl:
        ...
        for cf_path in args.cf_train_jsonl:
            cf_ds = CounterfactualDataset(
                cf_path, split="train", prompt_mode=args.prompt_mode
            )
    ...
    collate_fn = make_rag_collate_fn(
        tokenizer=tokenizer,
    ...
    # --- CF 验证集（可选）---
    cf_val_loader: Optional[DataLoader] = None
    if args.cf_val_jsonl:
        cf_val_datasets = []
        for cf_val_path in args.cf_val_jsonl:
            cf_val_ds = CounterfactualDataset(
                cf_val_path, split="test", prompt_mode=args.prompt_mode
            )
    ...
    total_steps_per_epoch: int = len(train_loader)
    total_steps: int = total_steps_per_epoch * args.epochs
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if (
                ...
                if accelerator.is_main_process:
                    logger.info(
                        "早停触发 (epoch end): %d 次验证未改善 (patience=%d)",
                        patience_counter,
                        args.early_stop_patience,
                    )
                early_stopped = True
    ...
    if accelerator.is_main_process:
        logger.info("=" * 60)
        logger.info(
            "训练完成! best_val_loss=%.4f, early_stopped=%s",
            best_val_loss,
            early_stopped,
        )
        logger.info("=" * 60)
